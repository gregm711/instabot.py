...
class InstaBot:
	...
	def follow(self, user_id):
		print("At follow in instabot.py")
			...
			try:
				...
				if follow.status_code == 200:
					...
					log_string = "Followed: %s #%i." % (user_id,
														self.follow_counter)
					self.write_log(log_string)
					self.save_to_txt(user_id)
				print("returning follow")
				return follow
			except:
	...
	def save_to_txt(self, user_id):
		with open("instagram.txt", "a") as text_file:
			text_file.write(user_id + " ")
			print("saved to instagram")
			return

		def unfollow(self, user_id):
			print("At unfollow in instabot.py")
			""" Send http request to unfollow """
			if self.login_status:
				if self.check_prev_followed(user_id) == False:
	...
	def check_prev_followed(self, user_id):
		print("checking prev followed")
		try:
			with open('instagram.txt', 'rb', 0) as file, \
			mmap.mmap(file.fileno(), 0, access=mmap.ACCESS_READ) as s:
				if s.find(str.encode(user_id)) != -1:
					return True
				return False
		except Exception as  e:
			print("failed at check instagram user id")
			print(e)
			return False
